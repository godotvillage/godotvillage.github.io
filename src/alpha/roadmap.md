...
- [ ] **精准代码行编辑** <Badge text="代码编辑" type="warning" vertical="middle" />
  - 目标：替换代码中指定行的内容
  - *技术方案*：使用 `TextEdit` 的 `insert_line_at(line: int, text: String)` 和 `remove_line_at(line)`
...
#### 设置与配置
- [ ] **全局设置系统**：将设置项改为全局生效
- [ ] **API Key 管理**：添加 Secret Key 设置项
- [ ] **自动展开思考**：添加配置项控制思考过程默认展开状态
- [ ] **智能引用配置**：添加引用后不自动填写文件名的开关
...
- [ ] **输入体验优化**：
  - [ ] 无输入时禁用发送按钮
  - [ ] 引用文件后自动在光标处插入文件名字符串 `TextEdit.insert_text_at_caret`
- [ ] **错误处理**：
  - [ ] DeepSeek 报错信息友好展示
  - [ ] 用户中断 AI 请求功能
